Feature: Legal structure retrieval by kvk number for anonymous users
  ...
  This is looked up against the KvK portal (where it should exist) and checked in CRM (where is should NOT exist).
  A new company record is then created for the parent organisation and all its subsidiaries.
  The corresponding legal structure is then returned, based on the data from the kvk portal and enriched with a relation ID for each member company.
